...
func (tq *TodoQuery) QueryOwner() *AutorQuery {
	query := &AutorQuery{config: tq.config}
...
func (tq *TodoQuery) WithOwner(opts ...func(*AutorQuery)) *TodoQuery {
	query := &AutorQuery{config: tq.config}
...
func (tq *TodoQuery) sqlAll(ctx context.Context) ([]*Todo, error) {
	...
	if query := tq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].autor_todos; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		query.Where(autor.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "autor_todos" returned %v`, n.ID)
			}
			for i := range nodes {
