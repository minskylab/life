...
import (
	...
	"github.com/minskylab/life/example/ent"
)
...
// The AutorQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type AutorQueryRuleFunc func(context.Context, *ent.AutorQuery) error

// EvalQuery return f(ctx, q).
func (f AutorQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.AutorQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.AutorQuery", q)
}

// The AutorMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type AutorMutationRuleFunc func(context.Context, *ent.AutorMutation) error

// EvalMutation calls f(ctx, m).
func (f AutorMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.AutorMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.AutorMutation", m)
}
